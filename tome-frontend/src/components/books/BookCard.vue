...
<template>
  <article class="group relative flex flex-col h-full bg-white border border-gray-200 hover:border-gray-300 transition-all duration-300">
    <!-- Бейдж скидки -->
    <div 
    ...
    <div class="flex flex-col flex-grow p-5">
      ...
      <!-- Название -->
      <h3 class="font-bold text-lg mb-3 line-clamp-2 flex-grow">
      ...
      <div class="flex items-center mb-4">
        <div class="flex">
          <svg v-for="n in 5" :key="n" class="w-4 h-4" :class="{'text-yellow-400': n <= Math.round(book.average_rating || 0), 'text-gray-300': n > Math.round(book.average_rating || 0)}" fill="currentColor" viewBox="0 0 20 20">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
          </svg>
        </div>
        <span class="text-xs text-gray-500 ml-2">{{ (book.average_rating || 0).toFixed(1) }}</span>
      </div>

      <!-- Цена и кнопка -->
      <div class="mt-auto flex items-center justify-between">
        <div>
          <div v-if="book.discount_price && parseFloat(book.discount_price) > 0" class="flex items-baseline">
            <span class="text-2xl font-bold">{{ formatPrice(book.discount_price) }}</span>
            <span class="text-sm text-gray-400 line-through ml-2">{{ formatPrice(book.price) }}</span>
          </div>
          <div v-else>
            <span class="text-2xl font-bold">{{ formatPrice(book.current_price) }}</span>
          </div>
        </div>
        <button 
